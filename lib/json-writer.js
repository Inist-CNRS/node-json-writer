...
JSONWriter.prototype.endElement = function () {
  ...
  if(this.stack.length == 0) {
    elt1[1][elt2_name] = elt2[1][elt2_name];  
  } else {
    if(elt1[1][elt1_name][elt2_name]) {

      if(elt1[1][elt1_name][elt2_name] instanceof Array) {
        elt1[1][elt1_name][elt2_name].push(elt2[1][elt2_name]);
      } else {
        elt1[1][elt1_name][elt2_name] = [ elt1[1][elt1_name][elt2_name], 
                                          elt2[1][elt2_name] 
                                        ];
      }
    } else {
      elt1[1][elt1_name][elt2_name] = elt2[1][elt2_name];
    }
  }
...
JSONWriter.prototype.startElementNS = function(prefix, name, uri) {
  ...
  if (!this.stack[0][1]['xmlns$'+prefix]) {
    this.stack[0][1]['xmlns$'+prefix] = uri;
  }
